...
- **NLC Improved（魔改新版 OGSR）上，部分超长文本字段渲染异常**：个别字段（例如 esc_attention 中的超长段落）在游戏内会显示为方块和西里尔字母，疑似引擎局部按 windows-1251 处理所致，具体机制未知。字库生成侧已验证无缺字、坐标无误。Golden Sphere OGSR（旧版）未复现。
