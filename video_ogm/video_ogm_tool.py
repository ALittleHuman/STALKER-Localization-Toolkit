...
# -*- coding: utf-8 -*-
"""
Video OGM Tool — 视频参数查看 & MP4→OGM 转换工具
支持拖拽，专为 STALKER 等游戏的 OGM 视频替换场景设计
"""

# ═══ 高 DPI 适配 ═══
import sys, os
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
from toolkit import T, apply_theme, apply_tk_defaults, color, LogBox, drop_zone
import ctypes
try:
...
class App:
    ...
    def _build_ui(self):
        ...
        ttk.Label(header, text="Video OGM Tool",
        ...
        from toolkit import drop_zone
        self.slot_source = drop_zone(slots, "源文件", "拖入 MP4 / MOV 或点击选择", on_file=self._on_source)
        self.slot_ref = drop_zone(slots, "参考 OGM（可选）", "拖入原 OGM 匹配参数", on_file=self._on_reference)

        # ── 信息面板 ──
        info_frame = ttk.LabelFrame(self.root, text="视频参数", padding=6)
        info_frame.pack(fill="both", expand=True, padx=MAIN_PADX, pady=(0, 4))
        ...
        # ── 输出路径 ──
        out_frame = ttk.LabelFrame(self.root, text="输出", padding=6)
        out_frame.pack(fill="x", padx=MAIN_PADX, pady=(0, 2))
        ...
        prog_frame.pack(fill="x", padx=MAIN_PADX, pady=(0, 2))
        ...
        bottom.pack(fill="x", padx=MAIN_PADX, pady=(2, 8))
    ...
    def _convert_thread(self, output: str):
        def prog(pct):
            self.root.after(0, lambda: self._on_progress(pct))
...
def main():
    ...
    if len(sys.argv) > 1 and os.path.isfile(sys.argv[1]):
        print(f"拖入文件: {sys.argv[1]}")
    App(ffmpeg, ffprobe).run()
