...
STALKER 模组文本提取器
======================
提取 gameplay 配置 XML 和 Lua 脚本中的可翻译文本。
替换为 string_table ID 引用，输出处理后的文件 + 独立 text XML。

用法:
    python stalker_text_extractor.py                 → 启动 GUI
    python stalker_text_extractor.py --cli <src> <dst> --prefix gs --clean  → CLI
...
# ═══ 高 DPI 适配 ═══
import ctypes
from toolkit import log_section, color, T as THEME, LogBox, apply_theme as _apply_theme
try:
...
RUS_LETTERS = "АаБбВвГгДдЕеЁёЖжЗзИиЙйКкЛлМмНнОоПпРрСсТтУуФфХхЦцЧчШшЩщъЫыьЭэЮюЯяЬ"
...
DEFAULT_ENCODINGS = ["cp1251", "cp1252", "utf-8"]
...
def _parse_cfgxml(filepath: str, quiet: bool = False) -> Tuple[str, Set[str], str]:
    try:
        from toolkit import read_text_file
    except ImportError:
        read_text_file = _local_read_text_file
...
def run_extraction(source: str, target: str, prefix: str = "mod",
    ...
    if clean:
        for sub in ["gameplay", "scripts"]:
            p = os.path.join(target, sub)
            if os.path.exists(p): shutil.rmtree(p)
        for name in [f"{prefix}_gameplay_texts.xml", f"{prefix}_scripts_texts.xml"]:
    ...
    ts = int(time.time())
    gp_prefix = f"sgtat_{prefix}_gp_{ts}"
    sc_prefix = f"sgtat_{prefix}_sc_{ts}"
    log(f"ID 前缀: {gp_prefix}")

    stats = {"gp_files": 0, "gp_strings": 0, "gp_chars": 0,
             "sc_files": 0, "sc_strings": 0, "sc_chars": 0}

    gp_src, gp_dst = os.path.join(source, "gameplay"), os.path.join(target, "gameplay")
    if os.path.isdir(gp_src):
        log("提取 gameplay 配置文本...")
        gp = _extract_gameplay(gp_src, gp_dst, gp_prefix, verbose=False)
        if gp:
            _generate_text_xml(os.path.join(target, f"{prefix}_gameplay_texts.xml"), gp)
            log(f"  gameplay: {len(gp)} 条 → {prefix}_gameplay_texts.xml")
        stats["gp_strings"] = len(gp)
        stats["gp_chars"] = sum(len(v) for v in gp.values())
    else:
        log("无 gameplay/ 目录，跳过。")
    if os.path.isdir(gp_dst):
        stats["gp_files"] = sum(1 for _ in Path(gp_dst).rglob("*.xml"))

    sc_src, sc_dst = os.path.join(source, "scripts"), os.path.join(target, "scripts")
    if os.path.isdir(sc_src):
        log("提取 scripts 脚本文本...")
        sc = _extract_scripts(sc_src, sc_dst, sc_prefix, verbose=False)
        if sc:
            _generate_text_xml(os.path.join(target, f"{prefix}_scripts_texts.xml"), sc)
    ...
    if os.path.isdir(sc_dst):
        stats["sc_files"] = sum(1 for _ in Path(sc_dst).rglob("*.script"))
...
class App:
    ...
    def _start(self):
        ...
        if not dst: messagebox.showwarning("提示", "请指定目标目录"); return
        if not os.path.isdir(src):
            messagebox.showerror("错误", f"源目录不存在：\n{src}"); return
        if not os.path.exists(os.path.join(src, "gameplay")) and \
           not os.path.exists(os.path.join(src, "scripts")):
            messagebox.showwarning("提示",
                                   "源目录中未找到 gameplay/ 或 scripts/ 子目录。"); return
        ...
        self._clear_log()
        self._log("═══ 开始提取 ═══", "dim")
        self._log(f"源: {src}"); self._log(f"目标: {dst}"); self._log(f"前缀: {pfx}"); self._log("")

        def worker():
            try:
                sts = run_extraction(src, dst, pfx, clean,
                                     log_callback=lambda m: self.root.after(0, self._log, m))
                self.root.after(0, self._on_done, sts, None)
            except Exception as e:
...
def _cli():
    ...
    i = 2
    while i < len(args):
        if args[i] == "--prefix" and i + 1 < len(args): pfx = args[i+1]; i += 2
        elif args[i] == "--clean": clean = True; i += 1
        else: i += 1
    sts = run_extraction(src, dst, pfx, clean, log_callback=print)
    print(f"\n{'='*50}")
    print(f"Gameplay: {sts['gp_files']} 文件, {sts['gp_strings']} 条, {sts['gp_chars']:,} 字符")
    print(f"Scripts:  {sts['sc_files']} 文件, {sts['sc_strings']} 条, {sts['sc_chars']:,} 字符")
    print(f"文本 XML: {pfx}_gameplay_texts.xml, {pfx}_scripts_texts.xml")
