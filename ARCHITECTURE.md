...
| `run_scroll_audit`：面板用法核对 | **不再钉死 `fit="content"`**，改成"`fit` 必须与里装的东西一致"：面板 body 里有这个 app 自己的 `CanvasTree`（弹性内容）→ 必须 `fit="viewport"`；静态内容 → 必须 `fit="content"`。★ `CanvasTree` 是**包装对象**（`.get()` 才是控件），判"里有没有树"不能用 `isinstance(子控件)`，得先把这个 app 里所有树的**控件**收出来再看在不在面板子树里 |
