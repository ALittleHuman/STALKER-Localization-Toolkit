...
**任务壳统一（2026-09-14）**：六个 App 的"忙碌标志 / 忙碌 UI / 线程启动 / 失败路径"
现在**全部**走 `toolkit_widgets.TaskRunner` —— 第 6 个是 `convert_app`，它此前自己养了
一套（`self.running` + `_busy_name` + `_start_worker` + `_end_task`）。界定写在这里：

| 归 `TaskRunner`（一份实现） | 仍归 App（共享壳不该管） |
|---|---|
| `running` 标志（**真正的重入闸门**：主线程同步置位，早于线程启动）、忙碌 UI（`on_busy`）、线程启动、失败路径（`on_error` **回主线程**执行）、进度起停钩子 | **暂停/继续**状态机（只有 convert 有：`pause_event` + 开始/暂停/取消三态）、**三种进度文案**（convert / restore / clear —— `TaskRunner.progress_update` 只会写 "done / total"，套上去会**改外观**）、"进度栏空闲时整栏收起" |

`convert_app._begin_task` 保留，但退化成"**拒绝时的用户可见提示**"（TaskRunner 不认识
任务名，给不出"已有任务正在运行：恢复备份"这种人话）；它与 `task.run()` 判断的是**同一个**
`task.running`，所以不存在两份状态 —— 这正是当初"套 TaskRunner 会出两份来源"那个顾虑的解。

锁：`run_app_probe.py` 新增一条（探针段 108 → **110**）——真跑一个可控慢任务，断言
`running`/三个按钮/拒绝提示，再跑一个抛异常的任务断言失败被报出来且忙碌态复位；
源码层禁止 `threading.Thread(` 自己起任务线程。**真任务端到端**另手工验过一次：
`start_thread()` 走完真实转换（临时目录里 windows-1251 → utf-8），产物 utf-8 无 BOM、
统计 `success=1`，结束时 `running=False`、按钮恢复、进度栏收回。
...
| 合并 motion + 冻结重窗格重绘（**档位已删除**） | **45 ms** | ≈ 30 fps |
| 只挪预览线（`LIVE_DRAG="line"`，**现唯一档位**） | 拖动期 4.5 ms，松手 102 ms | 拖动瞬间响应 |
| 只跑几何（`update_idletasks`，不含重绘） | 37.6 ms | — |
| 冻结整窗重绘（档位已删除） | 22 ms | 但连分隔条都不画，无意义 |

（表里 45 / 108 / 22 ms 是 2026-09-12 的实测记录，保留用于对比；对应档位已在
 2026-09-13 全部删除 —— 见下面的"现状"。）
...
**现状（2026-09-13 定稿）**：`LIVE_DRAG` **只有 `"line"` 一个档位**，语义三条：
①**合并运动事件**：拖动期只记最新位置、最多排一次应用，绝不逐事件重排（鼠标每秒上百个
motion，逐个处理会让分隔条落后光标几百毫秒，那才是"一卡一卡"的主观来源）；
②拖动期**一步几何都不改**，只画一块**填色带预览**（当前 sash 位置 → 目标位置，底色取
"被长大的那一栏"）再补一条 1 px 亮线标出精确落点；③**松手应用一次**
（`sashpos` → `_clamp()` 收敛 → 视口按新尺寸重排一次）。
用户裁定原话："那就还是不实时移动吧。松手一次重排。但是拖拽的时候预览框不能只是一条线，
而是要把那部分相应的背景也进行填充，不然很难看。"

**为什么把 `"freeze"` 与 `"full"` 两档删掉**：`"freeze"` 曾把栏目区留成空白（见下方历史
记录），用户明确否决；随后 `"line"` 成为定稿，剩下两档就成了**没有任何入口可达**的死代码
（`_frozen` / `FREEZE_MIN_WIDGETS` / `FREEZE_MAX_MS` / `_thaw_pane_redraw` / `WM_SETREDRAW`
看门狗 / `_on_destroy` 安全网），2026-09-13 整块删除（`toolkit_widgets.py` **-113 行** +
探针 2 段锁）。想复现"实时拖动"的对照数据只能从 git 历史里取。

> **历史记录（代码已删除，但教训保留）**：解冻**不能**只
...
> （该实现及其锁已于 2026-09-13 随冻结档一并删除；这条记录的意义是：
> **用"关掉重绘"换流畅，代价是用户可能看到空白界面，而这个代价不可接受。**）
...
### 6.1 渲染字号**按墨迹定标**（`fit_render_size`）—— 修掉"字比格子小 30%"

`CELL_HEIGHTS` / `BLOCK_WIDTHS` 是**格子**表（= 写进 ini 的 `height=` 与槽宽），它决定
行高与网格；**渲染字号**是另一件事。某个轮次把格子表换成了"汉化包生成工具2024 实测值"
（11→20 / 11→17，约 **1.3×**），但渲染字号仍取**槽位名**（11/13/15…）→ 格子对齐了参照包、
字没跟着长，字形比参照包小约 30%。用户实测感受就是"字比格子小、观感偏小"，并点名
"是不是某一轮改错了"。

修法（用户拍板 **A**：只动字，不动格子）：`fit_render_size(font_path, size, x2)` 在
`[size, size+14]` 里取**最大**的、且对样本（`FIT_SAMPLE`：常用汉字 + 全角标点）每个
**有字形**的字都满足
...
的档位；缺字形的字**跳过**（日文字体没有简体码位是正常的）；一个都量不出来就退回 `size`
（定标失败绝不崩、也不改变旧行为）。`render_font` 用它算出的字号渲染，`widths_of` 与
它共用 `rect_w_for` 这一条宽度真值。

> ★ 宽度那条**不是**"推进宽 − 2"：矩形框就是引擎的采样框 `[x1,x2)`，相邻字的框按推进宽
> 首尾相接，所以"墨迹正好填满框"既不裁墨也不越格。第一版多留了 2px 余量，实测每个槽位的
> 字都比参照包**小 1–2px** —— 用户报的"观感偏小"原样复发。这是**量出来的**，不是推的。

**A/B 实测**（同一字体：参照包自带的 `msyh.ttf`；同一批 86 个常用汉字；读 ini 的框 +
数 DDS 的真实像素）：

| 槽位 | cell | 汉字墨迹高 参照/我们 | 占 cell 参照/我们 | 墨迹宽 参照/我们 | 框宽 参照/我们 |
|---|---|---|---|---|---|
| 11 | 20 | 15 / 15 | 75% / 75% | 17 / 17 | 17 / 17 |
| 13 | 22 | 16 / 17 | 73% / 77% | 17 / 18 | 18 / 18 |
| 15 | 23 | 17 / 17 | 74% / 74% | 18 / 19 | 19 / 19 |
| 16 | 25 | 19 / 20 | 76% / 78% | 20 / 21 | 21 / 21 |
| 17 | 26 | 20 / 21 | 77% / 81% | 21 / 22 | 22 / 22 |
| 18 | 29 | 22 / 23 | 76% / 79% | 24 / 24 | 24 / 24 |
| 20 | 31 | 24 / 24 | 77% / 77% | 26 / 26 | 26 / 26 |
| 21 | 32 | 25 / 25 | 78% / 78% | 27 / 26 | 27 / 27 |
| 23 | 33 | 27 / 27 | 82% / 82% | 28 / 28 | 29 / 29 |
| 25 | 35 | 29 / 29 | 83% / 83% | 30 / 30 | 31 / 31 |

**墨迹高差：中位 +0px，范围 0…+1px；框宽 10/10 槽位一致。**（定标后字号放大 3–6 档，
原来按槽位名渲染时占比只有 50–66%。）
...
| 缺陷 | 复现（真数字） | 判据 / 修法 |
|---|---|---|
| **框宽可以超过 cell** | ① 表外的字按字体 advance+1 现算：msyh 的 `U+2116 №` 在槽位 23/25 得 **34/37**，而 cell 是 **33/35**；② 定标放大后 `clamp_box_to_ink` 还会把框抬到墨迹宽（同一码位达到 **36 > 35**）。推进宽就是引擎字距（`fTCWidth = l.z/vTS.x`），框宽一超格就**伸进邻格的采样框** | 三个参照包（FontGen 原版 / 汉化包生成工具2024 / NLC 现场包）逐条扫描：框宽 > cell 的记录数 **0 / 0 / 0** → `widths_of` 末尾 `min(rw, cell_h)`；抬框与封顶同时命中时（墨迹+内缩 > 整格）不越格、也不右裁，而是把墨迹**横向压进框**（`ink_draw_w`，实测只影响 № 这类极少数码位，压 1–3px） |
| **画布比行带矮一行** | 旧写法 `need = max_y - 1`：`max_y - 1` 恰是 2 的幂时正好少一行 —— slot 23（cell 33、单行）→ `tex_h=32 < max_y=33`。Pillow 的 `paste` 把行带最后一行**静默丢掉**（不报错、不计数），引擎采样越出贴图 | 引擎只按 `[y, y+height)` 采样，最后一行行号是 `max_y-1` → `need = max_y`。实测受害字：slot 23 的 `( ) [ ]`（墨迹正好填满行带，成品里底部整整 1 行像素没有）|

### 6.3 缺简体字形时用**繁体**顶上（`st_fallback.json`）

用户口径（2026-09-14）："日语的，标明是日语字体，然后那些缺简体的就上繁体。"

- **判据**：某码位在该字体里画不出（`getbbox() is None`）**且** `st_fallback.json` 里
  有繁体替代**且字体真画得出那个繁体形** → 用繁体字形画，**ini 的键仍是原码位**
  （引擎按原码位查表，换键等于没补）。
- **数据**：`font_pack/st_fallback.json` = OpenCC `STCharacters.txt`（Apache-2.0）的派生
  文件。派生规则：只收 BMP 内 CJK 单字；**繁体候选里含原字本身的条目一律不收录**
  （`干/后/里/台/表/丑/只` 这类字在繁体里照用原形，一刀切会把对的换成错的）；其余取第一
  候选。2640 条，37 KB。来源、许可、变换规则、源文件 sha256 都写在 json 的
  `_source/_license/_transform/_sha256_of_source_bytes` 键里。
- **实测覆盖**（现场那套 3801 字 × `hpsimplifiedjpan`）：缺字形 1296 个，其中 CJK 1115
  个 —— **880 个靠繁体补上了**，235 个补不了（连繁体形都不在日文字体里：`说→說` 的 `說`
  U+8AAA 就不在，日本用新字体 `説`），另 181 个是非 CJK（西里尔/特殊符号）。
  `build_package` 对后者**逐字告警**（不拒绝）："所选字体缺 N 个必需字形 … 显示空白"。
- **零影响**：只在"原码位真的画不出"时替换 —— 实测 msyh 在同一套 3801 字里替换数 **0**，
  锁里还进一步断言"把替换表清空后产物逐字节相同"。

**锁**（`run_functional_probe.py`，font_pack 组）：

| 锁 | 判据 |
|---|---|
| 墨迹按格子定标 | 样本最大墨迹高 ∈ [cell−8, cell−4]、每个字墨迹 ≤ 推进宽−2，且"再大一档必越界"（防搜索范围被写成 `range(size, size+1)` 这种假定标） |
| 字库画布高 ≥ 行带总高 | 每个槽位 × 档位 0/9 单字渲染后 `tex_h >= max_y` |
| 简繁替换表口径 | 表里 `汉/测/说/们/书/见/风/这` 逐条对得上，`干/后/里/台/表/丑/只` **必须不在表里** |
| 缺简体时用繁体补 | 9 个样本字在 jpan 下**原字真缺**、繁体真有 → 单字渲染的成品 DDS 框内**必须有墨迹**；ini 键必须是原码位、不得给繁体码位另开行；对 msyh 清空表后产物逐字节相同 |
...
且**拖动期不留任何未应用状态**（冻结档已删，见 §3.4；`LIVE_DRAG` 只剩 `"line"` 一档，
"三档合法"那类断言随之改成"拖的是**填色带**预览、拖动期 `sashpos` 不变、松手应用一次"）、
统一滚轮 3 条规则（`_on_wheel` 有条件 break / `ScrollViewport.try_scroll` 存在 /
路由里不许有「跳过内层控件」分支）、
...
7. **启动仍有的两项代价（明知而未修，理由在此）**：
   ...
   - ~~分帧装配期间每个栏目各占一帧，单帧最长 ~400 ms（视频转换，含 ffmpeg/ffprobe 解析
     334 ms），期间界面不响应~~ → **2026-09-14 已修**：视频页构造期**不再探测** ffmpeg
     （`find_ffmpeg()` 挪到动作入口 `_ensure_ffmpeg`，见 §11.5）。实测该页构造耗时
     **23~39 ms → 7.5~13.6 ms**（热；冷启动那一帧也从 162 ms 降下来），冻结包里省掉的
     是 ~373 ms（大头是 `import imageio_ffmpeg`）。代价：**第一次真正要用**时（选/拖文件、
     点转换）付一次 ~64 ms（源码版实测）。分帧装配本身仍在（首栏目同步、其余每帧一个），
     但现在没有哪个栏目会因为"探测 ffmpeg"而顶满一帧。
8. **分隔条跟手是"三档取一"，不是免费的（见 §3.4）**：Tk 每个控件一个 HWND，改尺寸就要
   重排/重绘整棵可见树。**含重绘**的实测（2026-09-12 记录）：逐事件实时 108 ms/次拖动风暴
   （≈12 fps）、合并 motion + 冻结重窗格重绘 45 ms（≈30 fps）、只挪预览线 4.5 ms（拖动期）。
   也就是说：拖动期**重页面内容不动**（松手一次补齐），换来拖动瞬间响应。
   想要"内容也逐帧跟手又不卡"，在 Tk 上做不到（要么把整页控件数降到
   几十个，要么换 UI 工具包）。**2026-09-13 定稿：只保留 `"line"` 一档**
   （拖动期画填色带预览、松手一次重排）——另两档（`"freeze"`/`"full"`）已整块删除，
   要对照数据只能从 git 历史取。
...
| `api.ui` | 后端无关的控件工厂，方法面固定 **16** 个：造控件 `label/button/row/checkbox/entry/text/combo/drop_box/pack` + 改控件/取输入 `set_text/set_state/text_view/alive/ask_yes_no/ask_open_file/ask_save_file`（`drop_box`/`set_state` 是 2026-09-13 面板改造补的，见 §11.4） |
...
"改控件 / 取用户输入"那 7 个方法是**迁移 `engine_utf8_patch` 时按实测补的**：插件真正需要的
除了"造控件"，还有"改标签文字 / 往只读视图写报告 / 判控件是否还活着 / 三个标准对话框"。
只给前 8 个时，插件仍得自己 `box.configure(...)`、`filedialog.askopenfilename(...)` —— 那还是 Tk 专有。
最后两个（`drop_box` 目标拖入框 / `set_state` 置灰）是 **2026-09-13 面板改造**补的（见 §11.4）。
...
| `engine_utf8_patch.py`（我方实验件，**已迁移**） | 面板在 Qt 宿主里 `panel/ok` 真建出来：目标拖入框（`QLineEdit` 只读 + `acceptDrops`，真接 DnD）+ 识别/生成/清空三个按钮（点击确实进 `identify`/`apply`）、1 个只读 `QPlainTextEdit`、按钮同一水平行、外层竖直顺序与 Tk 一致、无目标时"生成"不可点（两个后端跑的是**同一份**插件逻辑） |
...
> **上面这句"结构一个都没动"只对那次 `api.ui` 迁移成立。** 2026-09-13 的**面板改造**是
> **有意**改了结构与注册点（去掉重复的 toolbar 入口、去掉『校验』按钮、加目标拖入框），
> 相应的锁是**有意改写并逐条写了理由**的 —— 见 **§11.4**。

**锁**（下列数字是**当时**的口径；面板改造后的当前值见 §11.4）：`run_hub_probe.py` 新增 1 条
（Tk 侧 `api.backend == 'tk'` 且 `api.ui` 方法面完整、`api.widgets` 保留
—— 防止新契约在 Tk 路径上被悄悄丢掉）→ hub **42**；`run_ext_probe.py` 的插件段从 10 条加到 **11** 条
（新增"面板按钮点了真的进对应流程"：迁移后面板由同一个 builder 造，接线接错时
"command 非空 + 控件齐全"照样全绿 → ext **56**）；`run_qt_pilot_probe.py` 的 [8] 从 9 条扩到 **19** 条
（工厂方法面一致、**契约清单确实读到 14 个方法且不靠导入 Tk 模块**、Qt 工厂声明 `backend='qt'`、
真实插件注册期无错、闭源插件解密器可用、动作按钮生成、**真实面板 `panel/ok` 且无 fail 行**、
按钮齐全、只读结果视图、按钮同一水平行、外层竖直顺序与 Tk 一致、**点击真的进插件流程**、
...
| 契约清单读取返回空元组（Qt） | "契约清单是从 toolkit_plugin_ui.py 读到的 16 个方法…" |
| 面板『生成』接到 `identify`（**Tk 侧 `run_ext_probe.py`**） | "实验插件面板的按钮点了真的进对应流程（不只是 command 非空）" |
...
### 11.4 面板改造（2026-09-13）：去重、目标拖入框、识别即校验、契约 14→16

**用户口径（原话，勿改写）**：

1. "插件系统的UI布局是什么玩意，首先有重复的，其次为什么点击每一个按钮都会让我选择文件？有何必要？"
2. "直接就是和解包的DB栏一样啊，一个框（只不过这是装exe信息的，支持拖拽），几个按钮。点'识别'才会跳出选文件的窗口，如果没有文件就把其他几个按钮灰掉。"
3. "识别就已经自动'校验'了呀，所以这个按钮直接去掉就行了。补丁位置是按需打补丁，这样只要有收录，不管多少处漏洞都能给补了。"

**改了什么**（`plugins/engine_utf8_patch.py` + `toolkit_plugin_ui.py` + `qt_pilot_plugins.py`）：

| 项 | 改前 | 现在 |
|---|---|---|
| 入口数量 | 动作区三个菜单项 **＋** 面板一行三个按钮（同一组操作两遍） | **只有面板一处**；三个命令仍注册，但**零 toolbar 挂载点** |
| 面板控件 | 识别/校验/生成 + 清空 + 一行路径灰字 + 只读结果框 | 目标**拖入**只读框 + 识别/生成/清空 + 一行提示 + 只读结果框 |
| "选文件"次数 | 点每个按钮都先弹一次选文件 | 只有『识别』弹（1 次）；『生成』只问"输出文件"；拖入不弹 |
| 无目标时 | 三个按钮都能点 | 『生成』置灰（**真的点不动**），清空后重新置灰 |
| `api.ui` 方法面 | 14 | **16**（+`drop_box` 目标拖入框、+`set_state` 置灰） |

**为什么加两个契约方法、而不是让插件自己造控件**：照旧那条边界 —— 插件一旦自己
`import tkinter` 就重回 `tk-only`（`run_inject_layout_locks.py` 的注入 B 守着这条），
而"拖入"与"置灰"确实是面板需要的两种能力。两件都在**框架侧**实现：Tk 走 tkinterdnd2
（与解包页 / 视频页同源），Qt 用 `_DropLineEdit`（只读 `QLineEdit` +
`dragEnterEvent/dropEvent`，**真接 DnD**，不是"如实上报不支持"）—— 否则同一份插件代码
在两个后端能力不同，"可移植"就成了半截话。

**锁：有意改写，逐条给理由**（不是放松）

| 锁 | 改法 | 理由 |
|---|---|---|
| 动作区三项（旧 `ENGINE_TOOLBAR`） | 换成 `ENGINE_ACTIONS` + **两条反向锁** | 去重后动作区**不该**再有它们；注册表与真实渲染各查一遍，只查注册表证明不了"界面上真的没有" |
| 面板结构锁 | 期望按钮换成 识别/生成/清空，并**新增**"不得有『校验』按钮" | 用户明确要求去掉；加反向断言防止它悄悄回来 |
| 点击接线锁 | 期望 `["identify","apply"]`；点之前先给目标 | 新契约里"生成"无目标时点不动，不给目标就测不到接线 |
| **新增**：置灰/恢复锁 | 无目标→点不动；有目标→可点；清空→重新置灰 | 用户口径"没有文件就把其他几个按钮灰掉" |
| **新增**：拖入等价锁 | 拖入 → 目标被记下 **且**产出识别报告（不弹选文件） | 用户口径"拖入 = 点识别选文件"；只把 `_emit` 换成记录器，**不替换识别路径本身**，否则测的是"我把实现换掉了" |
| Qt 侧 | 契约 14→16；动作区按钮 3→**0**；按钮集合换新；新增"目标框接收拖入""无目标不可点/有目标可点" | 两个后端必须**行为一致**（跑的是同一份插件逻辑）。另把"报告里可移植贡献全部 ok"从"≥3 条 ok"改成"**没有 skip/fail 行**"——按条数钉住会把"有意去重"误判成回归 |

**红线没动**：这次改动**不碰判定逻辑** —— 判定仍只看逐目录项字节内容 + thunk 完整性，
字节数不进判定（`--selftest` 里的源码锁与命中矩阵锁照旧全绿）。

**同批完成的两件（改造方案 §四 的收尾）**：

1. **PE/RVA 核对降级**（方案第五节拍板项）：`.text 的 VA-RawPtr == 0xC00` 与"偏移都落在 .text 内"
   这两条**本构建特征**不再当拒绝条件，只出现在报告里（措辞明确写"**只是提示**"）——
   判定交给逐项内容 + `context` 锚 + 写后自检。硬查只剩"PE 能解析 / machine=x64 / PE32+"；
   目录项可以**显式**声明 `layout={"delta":…, "section":…}` 把它钉回来（收录别的构建时用）。
   顺带实测到一个事实：`rel32` 的起止两端用**同一个** delta，**天然抵消** ——
   布局根本不参与 rel32 计算，当初那条硬核对确实是纯特化。
2. **`apply` 原子写**：临时文件 → 逐字节复核 → 同盘 `os.replace` 改名；中途任何失败都只留
   "没有输出"，绝不留下半成品（用户下一步就会拿产物去替换游戏里的 exe）。自检里加了
   "不许留临时文件"这一条。

**验收（真机口径，与改前同一把尺子）**：真实 `xrEngine.exe`（sha `cb1d73…`）识别 = `原版`；
从它生成出的产物 sha **`67ddf4…` == 真机 `xrEngine_patched.exe`**、原版未被改动、输出目录无临时文件；
`--selftest` **68** 条全绿；`run_ext_probe` **57**、`run_qt_pilot_probe` **63**、
`run_ci.py` 全量 PASS；`run_inject_layout_locks.py` 全部符合预期（含改过的注入点）。

### 11.5 2026-09-14：视频页 ffmpeg 惰性化 + 两条"锁被注释/宽规则喂饱"的收拢

**① 视频页 ffmpeg/ffprobe 探测改惰性**（对应长期待办里的启动耗时一条）

* 现象：Hub 启动会**一次性构造 6 个 App**，而 `VideoOGMApp.__init__` 里同步跑
  `find_ffmpeg()` —— 冻结包里 ~373 ms（大头是 `import imageio_ffmpeg`），用户看到
  "打开就卡一下"（见 §3.3 第 7 条）。
* 改法（**两刀，同一天**）：
  1. **构造期不探测**：`__init__` 只建控件，`ffmpeg/ffprobe/converter` 先留空、
     `_ffmpeg_probed=False`；真正要用时才走 `_ensure_ffmpeg()`（幂等）——入口是
     **选/拖源文件、选参考 OGM、点转换**。
  2. **探测口径收窄**（用户当日改主意，原话："ffmpeg不用检不检测，直接读目录下的就行，
     读不到再跳出窗口让用户手动选择。"）：`find_ffmpeg()` 从"到处找（PATH / 常见目录 /
     imageio-ffmpeg）"收成**只认两处** —— 用户手动指定（`user.ltx`）+ **程序目录**
     （`app_dir()` 下的 `ffmpeg.exe` / `ffprobe.exe`）。
  3. **"自动安装"改成弹窗里的按钮**（用户第三条口径，原话："ffmpeg自动安装的按钮放在弹窗里。"）：
     读不到时弹一个**非阻塞小窗口**（`dialog_window`，自带「自动安装 / 手动选择 / 稍后」），
     自动安装能力**保留**但**只有用户点按钮才跑** —— `_start_auto_install` → 工作线程
     `_auto_install_ffmpeg` → 回主线程 `_apply_installed`（线程只做 IO/子进程，绝不碰控件）。
     安装**先试程序目录下项目自己的 `download_ffmpeg.py`**（它 ffmpeg + ffprobe 一起装、
     且显式拒绝 8.x），拿不到脚本才退回 imageio-ffmpeg（只带 ffmpeg，缺 ffprobe 就如实报差什么）。
     非阻塞（不 `grab_set` / 不 `wait_window`）还有一个好处：探针驱动页面时不会把进程卡住。
* 效果：`find_ffmpeg()` 从 **86 ms → 0.34 ms**（两次 `os.path.isfile` + 一次 user.ltx 读），
  而且"这次用的是哪个 ffmpeg"变成**可预期的一处**。版本安全性不变：程序目录里的二进制是
  `download_ffmpeg.py` **校验过**才落下的（它显式拒绝 ffmpeg 8.x —— 8.x 的 theora 编码器
  对 STALKER OGM 有 bug），实测本目录是 `7.1-essentials_build`。
* 状态栏诚实反映三态：**未检测 / ✓ / ✗**（探测之前不显示 ✗ —— 那是"找过但没找到"的意思）。
  读不到时由 `_ensure_ffmpeg` 经 `root.after(0, self._offer_ffmpeg_help)` 弹上面那个小窗口
  （延到事件处理之后，免得在拖放回调里开窗口）。
* 数字（源码版，同一台机）：该页构造 **162 ms（冷）/ 23~39 ms（热）→ 172 ms（冷）/ 7.5~13.6 ms（热）**。
* 锁（都在 `run_app_probe.py`）：
  ①构造**既不跑 `ffmpeg -encoders` 也不调 `find_ffmpeg`**，并且两边都要验"惰性没退化成
  永不执行"（旧锁在惰性化后会因 `converter is None` 整段跳过，等于自己架空 —— **有意改写、不是放松**）；
  ②**ffmpeg 只认两处**：AST 取 `find_ffmpeg` 函数体（剥掉 docstring）扫，不得出现
  imageio / locate_exe / extra_dirs / PATH 搜索，必须读 `app_dir()`，并真跑一遍核对来源。
  ★ ②的第一版按**全文**扫，被函数自己 docstring 里那句"不再 import imageio"喂饱后来又变红
  —— 与 §11.5 第 ② 条同一个教训（锁要抗注释/抗文档），改为只扫代码语句。
  ③**读不到时弹的是带按钮的小窗口，且「自动安装」只有点它才跑**：真建一次窗口核对三个按钮、
  点「自动安装」必须进 `_start_auto_install`、点「稍后」什么都不做；源码层再断言
  `__init__` / `_ensure_ffmpeg` 里**不得**出现自动安装（它只能挂在按钮上）。

**② `add_clipped` 口径收拢：一条被注释喂饱的源码锁**

* 旧锁（`run_hub_probe.py`）按**字符串**查：`"add_clipped" not in src` → 判"两个横向分栏页面
  都用了它"。而 `fs_app.py` 里 `add_clipped` 恰好只出现在**解释"为什么不用它"的注释**里
  （面板是 `ScrollPanel`、自带横/纵滚动条，再套一层裁剪会把底部连滚动条一起切掉），
  于是这条锁**永远为真**。
* 改成 **AST 数真实调用点**，并把两边口径分开写死：`fs_app.py` = **0 处**、
  `font_pack_app.py` **≥1 处**（实测 2）。锁名改为"横向分栏口径各就各位…"，
  探针还会打印实际计数，便于人看。

**③ 代理指标：从"人记着别用"升级成 AST 结构锁**（`run_ext_probe.py`）

* 规则**刻意写窄**（宽规则会误杀：`n_call_orig == len(call_ids) - 3` 是**项数**比较）：
  只盯"字节数"这一类名字 —— `changed_bytes` / `diff_bytes` 等**不得出现在任何比较或条件里**
  （只允许出现在给人看的报告文本里）；`EXPECTED_DIFF_BYTES` 这类常量名不得出现在可执行代码里。
* 注入验证当场抓出第一版的洞：文件里是 `fp["changed_bytes"]`（字典下标是**字符串常量**），
  而首版只收 `Name`/`Attribute` → 注入没能把锁弄红。补上"字符串下标键"后才真正生效
  （`run_inject_layout_locks.py` 第 ⑨ 条注入即为此设）。
* 探针条数随之从 **57 → 58**（`run_ext_probe.py` 的插件段 12 → 13 条）；注入验证从 13 → **14** 条。

### 11.6 2026-09-14：字体选择器四处实测缺陷（预览/换行/缺字形判据/记忆）

> 同日追加：判据③的 `missing` 之上再分出**第四态 `japanese`**（缺的简体字都能用繁体顶上），
> 界面要**标明日语字体**、预览直接用繁体字形显示 —— 见 **§11.7**。

用户一次报四条（原话见下），全部**先复现再改**：

> "预览，实际上只有那一个角落是能显示的。所以所谓缩放并没有什么卵用。以及文字描述为何换行如此拉跨。
>  同时，部分预览也不知道是为什么，会有常用字缺字体。然后 user 里面还要保存上一次使用的字体，
>  点一次『选择』保存一次。还有，不知道为什么，有些字的宽度这么大，远超实际字形宽度。"

| # | 复现出的事实 | 根因 | 改法 |
|---|---|---|---|
| ① 预览只占一角、缩放没用 | 预览图**恒为 330×130**：`Image.new("RGB", (330, 130))`；放大只放大**字号**，字一超过 330×130 就被裁掉 | 图尺寸与内容/画布无关 | 按 `font.getmetrics()` + `getlength()` 量出文字尺寸出图（宽 = max(文字宽, 画布可用宽) + 2×pad），并设 `scrollregion` —— 放大 = **整张图变大**，滚动条可用 |
| ② 描述文字一字一行 | 复现：`wraplength` 落到下限 **80px** | 绑在它自己的 `<Configure>` 上取 `winfo_width()`，布局早期那是 1 → 被 `max(80, …)` 兜死 | 改由**容器宽度**驱动（`pv.winfo_width()-16`，下限 160），并在 `after_idle` 里补一次；实测真窗口：窗格 641 → wraplength **625** |
| ③ 标 ✓ 中文却缺常用字 | 复现：`hpsimplifiedjpan-regular.ttf`（**日文版**）有 永/你/好/化，**没有** 汉/测/试；老判据 `if b is None: continue` 把缺的字**跳过** → "不同 bbox 数 4 > 1" 判 ✓ | 判据对"缺字" fail-open | 样本换成**含简体特有字**的 `永你好汉化测试这说们书见风`；缺一个即 `missing`（fail-closed），"bbox 全同"（notdef）仍算缺失。复验：jpan→missing(缺 汉测试这说们书见风)、simsun/msyh→ok、Harlow/HP Simplified Rg→missing(bbox 全同) |
| ④（⑤的来源）"有些字宽度远超字形" | 复现：jpan 里 `汉` 的**推进 40px、墨迹 0** | 字体真的没有该字形时，PIL 仍按 em 推进 → 一个**整字宽的空洞** | 不是渲染坏了：预览标题现在直接点出"缺：汉测试这说们书见风"，把"为什么"写在界面上 |
| ⑤ 字体不记忆 | `font_pack_app` 里**没有任何** `user.ltx` 读写 | 只在内存 | 启动优先读 `user.ltx` 的 `[font] path`（文件仍在才用）；点『选择』（含"浏览其他目录"）即 `save_user_values("font", ...)` —— 与视频页 `[ffmpeg]` 同一机制，会保留其它段 |

**锁**：`run_app_probe.py` 新增"字体选择器"一条（app **110 → 111**）—— 用真弹窗断言
①放大后图变宽、②`wraplength` 跟随窗格（不落 80）、③jpan 必须 ⚠ 且标题点出缺哪些字、
④点『选择』后 `app_dir()/user.ltx` 真的出现 `[font]`（把 `toolkit_platform.app_dir` 指到临时目录，
不污染真实配置）。★ 这条锁自己踩过两次坑、都是**锁写错**而不是被测代码错：
"取第一个 Canvas"取到了 `add_clipped` 视口自己的 Canvas；`isinstance(w, tk.Button)` 漏掉 ttk 按钮
—— 两条都记在这里，免得下次再写一遍。

---

### 11.7 2026-09-14（同批）：字库定标 / 两处几何缺陷 / 简繁补字 / 日语字体标注

用户一轮话里给了三件事，逐条对应到实现与锁（上文 §6.1–6.3 是引擎侧的口径）：

> "日语的，标明是日语字体，然后那些缺简体的就上繁体。我给你的目录，大小是+9。我的屏幕是
>  2.5K，所以使用+9会偏大。但事实是观感偏小，所以应该是需要调整墨迹大小（自己一个一个核对
>  清除，我不希望再返工。你也可以和以前代码对比一下，是否真的如我所说，在某一轮修改中把
>  这个改错了）。"

| # | 用户说的 | 查出来的事实 | 落点 |
|---|---|---|---|
| ① "观感偏小…是不是某一轮改错了" | 属实：格子表被换成 1.3× 的参照实测值，渲染字号仍按槽位名 → 字形小约 30% | §6.1 `fit_render_size` |
| ② "自己一个一个核对" | 用真渲染逐槽位量墨迹（不是查表）：msyh 放大 2–4 档、墨迹高 14/20 … 28/35 | 锁"墨迹按格子定标"（判据含"再大一档必越界"） |
| ③ "那些缺简体的就上繁体" | 现场那套用日文字体时 1115 个 CJK 缺字形，880 个能用繁体补上 | §6.3 `st_fallback.json` + `render_font` 补字 |
| ④ "标明是日语字体" | 选择器新增第四态 `japanese`（与 `missing` 分开），列表标记与预览标题共用同一判据；预览行直接用繁体字形画（否则还是空白） | `apps/font_pack_app.py` |
| ⑤ "大小是+9" | 现场包 height 31/…/48 = `cell_height_for(槽位+9)`，确认是**本工具**带「尺寸 +9」产出（此前我判断成"不是本工具产物"，是错的） | 无需改代码 |

**锁与注入**（本轮）：

| 位置 | 新增 | 结果 |
|---|---|---|
| `run_functional_probe.py` | 4 条：墨迹定标 / 画布高 ≥ 行带总高 / 简繁表口径 / 缺简体用繁体补（含"全字形字体零影响"字节比对） | **112 PASS / 0 FAIL** |
| `run_app_probe.py` | 字体选择器那条**扩写**：jpan 必须标"日语"、标题必须给 `简体→繁体` 对应 | **111 PASS / 0 FAIL** |
| `run_inject_layout_locks.py` | 注入 **14 → 21** 条（新增：定标退化成按槽位名 / 定标上限被拆 / 画布少一行 / 框宽不封顶 / 表被清空 / 无条件替换 / 界面退回旧标签）。（构建器那 3 条见 §11.8，合计 **25**） | **21/21 全部符合预期**，还原逐字节一致 |

> ★ "定标"这类**修复型**改动最容易被写成"看起来在测、其实恒真"的锁 —— 所以定标锁的判据里
> 特意加了"再放大一档必须越界"（把搜索范围写成 `range(size, size+1)` 会立刻变红，注入⑮即是），
> 以及"全字形字体清空替换表后产物逐字节相同"（防止替换逻辑顺手改了不缺字的字体）。

---

### 11.8 2026-09-14（同批）：构建器 —— 序号跟着标记走 + 构建输出的横向滚动条

> "构建器，在切换标记的时候，序号也要自动改。还有，滚动条呢？"

| # | 用户说的 | 查出来的事实 | 改法 |
|---|---|---|---|
| ① | 切标记，序号也要自动改 | 键是 **(完整版本串, 标记名)**，序号各数各的（仓库当前 `1.1.0|ALPHA`=32，`BETA` 靠"已保存标记"给的下限 = 1）。旧实现 `_on_marker_pick()` 只置"手动优先"，**输入框里还是上一个标记的号** → ALPHA→BETA 会给出 `BETA.7` 这种从没发过的号 | 选完标记即按 `VER.next_prerelease(tag=新标记, consume=False)`（与 `build.py` 自增时**同一个**函数）回填序号；手动语义不变（原样构建、计数不动）。版本号同理 —— 它也是键的一半：自动模式下改版本号就按新键重算（`9.9.9` → ALPHA.1），**手动模式绝不动**用户手填的号 |
| ② | "还有，滚动条呢？" | 构建输出是 `LogBox(wrap="none")`，而 LogBox **只建竖条**。真窗口实测 `xview()=(0.0, 0.06)`：94% 的行宽看得见却**没有任何可拖的东西**（PyInstaller 的路径行就是那么长） | `LogBox(..., hbar=True)` 新增**可选**横向滚动条（默认不建 → 其余调用点零影响）。两条条子与文本仍在**同一个父容器**里用 pack 排，不新建内嵌 frame —— 调用方（构建器）拿到 LogBox 后还会再 pack 一次，换容器会把文本搬出内嵌格 |

**锁**（`run_build_probe.py` 的构建器 GUI 段 112 → **115**）：

| 锁 | 判据（都是实测，不是查代码） |
|---|---|
| 切换标记时序号跟着这个标记走 | 先塞一个**与期望值必然不同**的残留号（期望 +100），再走下拉真实的 `<<ComboboxSelected>>` 回调；序号必须等于 `next_prerelease(tag=新标记)` 的 peek 值；选「无」必须回到 1 且 `_marker_args()` 传空串 |
| 改版本号（新键）时序号按新键重算 | 自动模式：版本改成 `9.9.9`（仓库里没有的键，避开"已保存标记给的序号下限"）→ 序号必须回到 1；手动模式：改版本号**不得**动用户手填的号，`_marker_args()` 仍原样传它 |
| 构建输出有横向滚动条且真接线 | 同一容器里横竖各一条 `AutoScrollbar`；内容真的横向溢出（`xview()[1] < 1.0`）；**两个方向都真接线** —— 文本 `xview_moveto` 后条子 `get()` 跟上、调条子的 `command moveto` 后文本真的横移；条子有几何管理器且有真实尺寸（不是 1×1 的假可见） |

**真机截图**（`%TEMP%` 里的一次性脚本开真窗口 + `PrintWindow`）：窗口底部出现横向滚动条、右侧竖条在位、超长行可拖。
...
### 11.11 2026-09-15（追加）：两条自伤回归、操作栏被切、"不该滚的时候也滚"

用户连问三次，**前两问都是上一批改动造成的**（先记结论：这一节的主体不是新功能，是回归）：

> "不是，我竖的滚动条是咋回事？构建器的滚动条呢？一天天干啥吃的。"
> "下面按钮没显示，但是没有滚动条。啥情况？还有DB栏滚动条又是啥情况？你到底在干嘛？"
> "不是竖条的问题，是不该滚动的时候也滚。"

**① 树/日志的两条滚动条 pack 顺序（`c55c06d` 的自伤）**

我给 `CanvasTree` 加横条时，把它 pack 在 `expand=True` 的画布**之后** —— 画布先吃掉剩余空间，
横条只能分残渣。真机实测（`FSToolApp`，读 manager/宽高/ismapped）：

| | 横条 | 竖条 | 后果 |
|---|---|---|---|
| 改前（错顺序） | **44x15 且常驻可见**（装得下也不收） | 面板多出一条 | 白占 15px 高 → 面板内容被顶出视口 → **面板自己的竖条也冒出来**（用户看到的"竖条不对劲"） |
| 改后（`c5ffc23`） | 装得下时 manager=`""`、ismapped=0；超宽时 **812x15** 撑满 | 装得下收起；超高时 **15x540** 撑满 | 每个面板只剩**一条**可见竖条 |

pack 顺序固定为 **竖条(右,满高) → 横条(底) → 被滚的控件(占剩余)**；`LogBox` 一直是对的，
`CanvasTree` 是我改坏的 —— 回归只发生在树，不在日志，所以"日志看起来正常"不能当证据。

★ 顺带发现**夹具本身会造假**：探针根窗口里塞了六个 App 的页面，再往根上 pack 固定尺寸夹具会被挤成
1x1 → 画布恒"装不下" → 判据全假。夹具改放**独立 `Toplevel`**（同 §7.x 的老坑）。
★ withdrawn 的窗口里控件恒 1x1，量之前必须 deiconify。

**② 构建器操作栏（开始构建…）被切**

用户看到的"下面按钮没显示，但是没有滚动条"＝操作栏整条被 unmap，实测 `y=0 h=1 ismapped=0`。
根因仍是 **pack 的调用顺序**：日志区（`expand=True`）先 pack，操作栏最后 pack ——
`expand` 只在"所有兄弟先拿到请求尺寸之后"分剩余，所以窗口一矮操作栏就分不到。
改法：操作栏**先 pack、`side="bottom"`**，把 56px 预定下来，剩下的才给日志区
（日志区自己有滚动条，被压缩还能用）。150% 屏实测：

| 窗口高（物理） | 操作栏 | 日志区 |
|---|---|---|
| 1020（默认） | y=952 h=56 ✓ | 368 |
| 900（600 设计高） | y=832 h=56 ✓ | 248 |
| 840（minsize） | y=772 h=56 ✓ | 188 |

三个按钮（开始构建/打开产物目录/停止）全部 mapped 且有真实高度；日志区没被压没。

**③ "不该滚的时候也滚"（本轮的正题）**

用户先给了两张裁边截图 + "看视频"，我按前两轮的思路去找竖条 —— 错了。用户的判据是：
**不是竖条的问题，是不该滚动的时候也滚。** 现场量化（真窗口 + 真 `FSToolApp`，
`E:\Human\NLC Improved [OBT]\gamedata.sq_base` 按正常路径 `_load` → `_rebuild_merged`
→ `_render`）：

| | 面板 body req / 视口 | 面板自己的 `yview()` | 树 |
|---|---|---|---|
| 改前（面板 `fit="content"`） | 锁的夹具 **156 / 63** | **(0.0, 0.4038)** —— 明明没内容却在"能滚"状态 | 首行还能被面板拖到视口底部 |
| 改后（`fit="viewport"`） | **63 / 63** | **(0.0, 1.0)**（真机两面板同为 `(0.0, 1.0)`，面板内可见竖条 **0** 条） | 数据包列表 0 行 → 树也没条；包内文件 15 行 → **树自己的竖条 15x333 出现**（该滚的还是能滚） |

同一张截图里能同时看到两件事：左边"数据包列表"空着且**没有**任何滚动条（不该滚就不滚），
右边"包内文件"15 行装不下 → **树自己**的竖条在（该滚的由树的滚动条负责）。
全窗口 10 条 `AutoScrollbar` 的真实状态（`winfo_manager()`/`winfo_ismapped()`）：
只有 `file_tree vertical mgr=pack map=1 15x333` 一条是显示状态，其余 9 条全部 `mgr=- map=0`。

机制（写在 `ScrollViewport._sync` 里）：`fit="content"` 时滚动区高度取
`max(内容 reqheight, …)`，而树有**最小请求高 px(120)=180**，面板视口矮到 63 时
**180 > 63 → 明明没内容也滚得动**；`fit="viewport"` 时滚动区高度 = 视口高 → 面板自己不滚，
"行多了怎么办"交给**弹性树自己的竖条**（树 `expand=True` 占满内容区，行数超出时才出条）。
于是同一页里"面板滚 + 树滚"两条滚动路径**只剩树那条**，口径唯一。
两处 FS 面板（`数据包列表（.db / .sq）` 与 `包内文件`）都改 `fit="viewport"`。

★ 这条我先按"竖条"方向查了一轮（还发了截图），方向是用户纠正的。教训：用户裁边截图 + "看视频"
时，**先把他的判据原话当判据**，别急着猜他指的是哪个部件。

**★ 用户第二次报同一件事（"你真的看视频了吗？我滚动，然后 DB 栏没有任何滚动条的自己滚起来了"）**

我没看画面就去猜部件，这次回去**逐帧量了那段录屏**（`ffmpeg -vf fps=10` → 96 帧 + 模板匹配定行位）：

| 量到的事实 | 值 |
|---|---|
| 「数据包列表」里那一行（`☑ gamedata.sq_base [已加载] 16.9 MB`）的纵向位置 | 帧 11~27 / 49~61 / 68~96 一直是 **y=722**（面板底边 ~y=790）—— **贴在面板底部**，整段视频里没动过 |
| 该面板的纵向滚动条 | 右侧只有一条**很短的滑块**（≈20px）—— 说明面板当时是**可滚**状态 |
| 面板高度（视频） | 约 280 逻辑像素（真机 150% → 物理约 420） |

⇒ 视频里的状态就是：**面板可滚 + 那一行被摆在面板底部**，也就是"没有条也能滚"的现场。
按同一窗口尺寸（1700×1010 逻辑，真 Hub 结构 + 真包 `gamedata.sq_base`）做**窗口高度扫描**（改前/改后）：

| 窗口高（逻辑） | 改前：面板能滚？ | 改后：面板 `yview` | 改后：行文字 bbox.y | 改后：滚轮动了几个画布 |
|---|---|---|---|---|
| 1010 / 900 / 800 / 700 / 640 / 580 / 520 / 470 / 430 | 面板自身短于内容时能滚（探针实测 `yview=(0.0,0.4038)`） | 九档全部 **(0.0, 1.0)** | 九档全部 **5**（贴着工具条下方） | 九档全部 **0** |

同一张真机截图（`fs_viewport_shot.png`）里也直接能看到：左边面板空着且**没有**任何滚动条，
右边"包内文件"15 行装不下时**由树自己的竖条**负责 —— "不该滚的不滚、该滚的还能滚"。

★ 记一笔我自己的翻车：这次"查条子"时我先写了个**启发式**找条函数（按"画布 master 子树里的
`AutoScrollbar`"猜），一次跑出 **50 条假阳性**（连"装得下却占着条"都被误判）。锁不可信比没锁更坏，
已改成**只查契约明确的控件**（`ScrollViewport` 的 canvas + 它自己的 vbar/hbar；`CanvasTree` 的
canvas + 它自己的 vbar/hbar），不猜、不抽样。

**★ 为什么"全量审计"没抓出来（用户追问："为什么我之前让你做全量审计查不出来？"）**

`run_scroll_audit.py` 一直在跑、一直是绿的，原因有两条，都在它自己的判据里：

| # | 洞 | 证据 |
|---|---|---|
| ① | 判据只有**单向** | 它只查"内容显示不完全 ⇒ 必须有条"（`check_one` 的 `ovf_x/ovf_y` 分支）与"装得下 ⇒ 条必须收起"。**反向不变量"没有可滚内容 ⇒ 绝不许能滚"从来没写进去过** |
| ② | 它把**错的配置当契约钉住** | `audit_panel_wiring` 当时断言 `view._fit == "content"` —— 也就是说：这个缺陷在审计眼里是"合规"，我去修它反而会被审计报红（本轮 CI 就真的先报了两条 `面板用法核对：... 不是 horizontal=True/fit=content`） |

所以它**结构上不可能**抓到这个缺陷，还会替它站岗 —— 这也是"几轮统一"里最贵的一条：
我每轮核对的都是**形状统一**（都是 `AutoScrollbar`、都会智能隐藏），不是**规则统一**
（没有可滚内容就不许滚）。规则从来没进过任何一条判据。

改法（两条锁，各放在**能复现机制**的地方）：

| 锁 | 位置 | 判据 | 能红吗 |
|---|---|---|---|
| 契约级 | `run_scroll_audit.audit_all_panels` | 走树扫页面里**所有** `ScrollPanel`（app 无关；"弹性内容"按"body 子树里有没有 `AutoScrollbar`"判，因为面板自己的条挂在 `ScrollViewport` 上、不在 body 里）：body 里自带滚动条 ⇒ 必须 `fit="viewport"`，否则必须 `fit="content"` | ✓ 实测注入后 `违约 1 个` + 具体那面板的路径 |
| 行为级 | `run_app_probe`（矮窗口夹具） | `yview()==(0.0,1.0)`、`try_scroll` 不吃、强滚不动、首行贴顶 | ✓ 注入后 `yview=(0.0, 0.4038)`（body req 156 / 视口 63） |

★ **反向不变量**（"没内容不许能滚"）原本也写进了 `run_scroll_audit`，结果发现它**永远不可能红**：
本审计是 Hub 式布局，页面视口 `fit="content"` + `minsize_y=px(430)` 让每个 App 恒定拿到自己的
**自然高度**，面板根本压不矮 —— 实测把窗口压到 **900x320**，fs 面板仍有 **396px** 高、
`yview=(0.0,1.0)`，注入缺陷也照样绿。**死锁比没锁更糟**，所以那条判据挪到能复现机制的
"裸 Toplevel + 矮窗口"（视口 63px < 树最小请求高 180px）里，本审计只留契约级那条。

**锁**（本轮新增/加强）：

| 锁 | 判据 |
|---|---|
| `run_app_probe`：树的横/竖滚动条撑满、该收就收、该出就出（含两向接线） | 装得下 → manager=`""`+ismapped=0；超宽/超高 → 真实尺寸撑满；`xview_moveto`/`yview_moveto` 后条子 `get()` 跟上；调条子 command 后画布跟上 |
| `run_build_probe`：构建输出（`wrap=none`）的横/竖滚动条该收就收、该出就出且撑满 | 同上；参照物取**文本自己**（LabelFrame 边框+标题占约 46px，拿它当参照永远差一截） |
| `run_build_probe`：操作栏（开始构建…）在最小窗口下也不被切 | 默认与 minsize 两档：操作栏 mapped + 真实高度 + 下沿在窗口内，日志区没被压没，三个按钮都在 |
| `run_app_probe`：**fs：弹性树的面板在没有可滚内容时不许滚（单行贴顶部）** | 真建 `FSToolApp`（独立 `Toplevel`）→ 断言 `panel.view.canvas.yview() == (0.0, 1.0)`、`try_scroll(假事件)` 为假、强行 `yview_scroll` 后仍不动、首行落在视口上半；树的竖条只在画布 ≥ `ROW_H+2` 时才算"该出" |
| `run_scroll_audit`：面板用法核对（**全页面，类级**，`audit_all_panels`） | 走树扫页面里**所有** `ScrollPanel`（不点名、不按 app 硬编码）：body 子树里自带 `AutoScrollbar`（= 内容自己会滚，弹性）⇒ 必须 `fit="viewport"`；否则必须 `fit="content"`。实测查 3 个面板、违约 0 |
| `run_scroll_audit`：面板用法核对（fs 两面板的点名版） | 与上面同规则，另打印窗格 minsize 的真实落值 |
| `run_scroll_audit`：**两向不变量**（`audit_two_way`，本轮新增） | 用户判据的直接编码："**能滚 ⇒ 必须看得到条；装得下 ⇒ 不许占着条**"。查每页的**页面视口 + 所有面板视口 + 所有 `CanvasTree`**（各自用自己的 vbar/hbar，不猜）。注入"竖条永不 pack" → 六页里的可滚视口全部报 `能滚却看不到条` |
| `run_scroll_audit` 覆盖面 | 从"fs 一页 + 夹具"扩到**全部六个栏目**（按 Hub 真实结构建页：页面视口 `fit="content"` + `minsize_y=page_min_h()`）；用户原话"其他小窗口也是一样" —— 只查一页等于抽样推广 |

**注入**（32 → **36** 条）：`hbar=True`→`False`、`_content_w` 退回 0、pack 顺序打回错的那版、
操作栏被大请求高的兄弟先占、**弹性树的面板退回 `fit="content"`**、**竖条永不 pack（能滚却看不到条）**
—— 各自把对应锁弄红。
★ 注入器现在支持一条注入跑**多个**探针：㉞ 同时钉在 `run_app_probe`（行为锁）与
`run_scroll_audit`（用法核对），两个探针都要抓到才算这条注入成立。

★ 两条注入本身的教训：㉝ 第一版插了个**空 SplitPane**（请求高很小，根本没挤到操作栏）
→ 探针全绿 → 注入器当场报"锁没抓住这个错"；㉞/㉜ 涉及 `toolkit_widgets.py` 必须写 `\r\n`
（该文件是 CRLF，多行锚点里的 `\n` 匹配不上）。**注入必须真的复现那个机制才算数。**
★ 注入器加了**启动自检**：目标文件里若还留着 `# 注入` 文本（上一次被强杀、`finally` 没跑到）
就拒绝启动（exit 2）并提示 `git checkout --`。这一版之前我一次前台超时被砍，
`font_pack.py` 里就留下了注入文本，下一次会把脏版本当基线。

**门禁**：`run_ci` 全量 PASS（app 116 / functional 114 / build 116 / hub 76 / ext 58 /
qt 63 / qtbuild 11 / ui 13 / eng 8 / pyflakes 0）；注入 **35/35** 全部符合预期。
**真机截图**：`fs_viewport_shot.png`（FS 页：数据包列表空着且**无**任何滚动条、包内文件 15 行
装不下时**树自己**的竖条在位）、`fs_viewport_crop.png`（放大面板下沿：没有残留的条，
只有窗格 sash）、`builder_shot3.png`（构建器底部操作栏三个按钮都在）。
...
### 11.13 2026-09-17：**任务结束后不许停在"…中"**（用户："加载上DB之后也显示'加载中'"）

> "不知道为什么，加载上DB之后也显示'加载中'（加载的是sq，其他db没试过）。"

**根因**（不是加载失败，是收尾没写状态）：`TaskRunner.run(status="加载中…")` 把状态写成运行期文案，
而 `_finish()` 只在**调用方显式给了 `final_status`** 时才改写它 —— 六个 App 里只有
`font_pack`（`final_status=("完成","ok")`）与 `fs_app` 的**封包**那条传了；
`fs_app` 的**加载/扫描/解包**全都没传，而它的成功路径**一个字都不写**（失败路径反而会写
`"加载失败: …"`）→ 状态栏永远停在"加载中…"，只有成功时才露出来。

**修法（结构性，不靠每个调用点记得传）**：

| 落点 | 改动 |
|---|---|
| `TaskRunner.run()` | 记下**任务开始前**的状态（`_prev_status`）；同时记住本次写进去的运行期文案（`_running_status`） |
| `TaskRunner._finish()` | 若"没给 `final_status`、没有错误状态、且状态栏还是 `run()` 写的那句" → **恢复开始前的状态**。App 自己在完成回调里 `set_status`（text_extract/video/xml 都这样）时文案已变，**不会被覆盖**；`on_error` 写的失败状态依旧优先 |
| `TaskRunner.__init__(idle_status=…)` | ★ 恢复时要拿它兜底：`_status_text` 在 App 从没写过状态时是**空串**，而标签上的"就绪"是**控件构造时**写的 —— 两者不同步。第一版没兜底，实测加载完之后状态栏变成**空**。各 App 空闲文案不同：fs / text_extract / video = `"就绪"`（已传），font_pack / xml = `""`（默认） |

**锁**（`run_app_probe`："任务壳：收尾后**不许停在「…中」**"）三条都验：

| # | 判据 |
|---|---|
| ① | 谁都没写收尾文案 → 标签回到**任务开始前的文案**（探针故意**不同步** `_status_text`，模拟真实 App：标签"就绪"、TaskRunner 内部空串） |
| ② | `on_error` 写的失败状态优先，不被恢复覆盖 |
| ③ | App 自己在完成回调里写的"完成 · 3 条"优先，不被覆盖 |

**注入**（37 → **38** 条）：拆掉"恢复前置状态"分支 → ① 立刻红，报的正是用户看到的那句
`任务结束后仍停在运行期文案：状态栏='加载中…'`。

**真机端到端**（真程序 + 用户自己的 `gamedata.sq_base`，17MB，走 NLC 解密插件）：
装载前状态栏"就绪" → 任务 `running=False`、**状态栏回到 `'就绪'`**、`已加载包=1`、
包内文件树 15 行、两面板 `yview=(0.0,1.0)`、首行 `bbox.y=5`（贴顶）。

★ 顺带两条实测：①`gamedata.sqz_*` 两个包**不在** `NLC Improved [OBT]` 根目录（在子目录里），
我第一版复现脚本把路径写错，日志报"无法识别 .sq 文件" —— **是夹具错，不是产品错**；
②当天 `sqfs2tar.exe` 已能被 360 放行（用户点了信任），`sqfs_list` 用 40KB 真镜像复测：
5050 项 / 0.22s / **4999 个非零大小**（唯一的 0 是那个真的空文件）→ "大小全 0 B"确系 AV 拦截所致。

### 11.14 2026-09-17：**SquashFS 大小改由纯 Python 读元数据**（选项 B 的第一步，不再依赖 `sqfs2tar`）

用户的决定是"**B：去掉这个依赖**"。落地时才发现三条工具路都拿不到精确字节（逐个量过）：

| 路子 | 为什么不行 |
|---|---|
| `rdsquashfs --describe` | 有类型/路径/权限，**没有大小** |
| `rdsquashfs --list <目录>` | 大小是**人读单位**：`4k`（真实 3823 B） |
| `rdsquashfs --stat <文件>` | 精确，但**一文件一次进程**（实测 71.6 ms → 5000 文件 ≈ 6 分钟，19642 文件 ≈ 23 分钟） |
| `sqfsdiff --old 空镜像 --new 真镜像` | 只打印 `> 路径`，**不带大小** |

于是只能自己读：真实镜像 = **明文 `hsqs` + LZ4(5) + 128KB 块**（只有 `gamedata.sq_base` 加密，由 nlc_sqfs 插件解密）。实现落在 `file_system/stalker_fs.py`：`_sqfs_lz4_block` / `_sqfs_decompress`（gzip、xz、zstd、lz4）/ `_SqfsMeta` / `_SqfsCur` / `_sqfs_read_inode` / `_sqfs_walk_sizes` / `_sqfs_meta_sizes`。

**三个必须记住的格式坑**（都是踩出来的，写在代码注释里）：
1. 元数据块的 **2 字节头在块起始处**（头 bit15=未压缩、低 15 位=压缩长度）—— 我先写成 `+2`，于是把数据当头读，gzip 解出垃圾、lz4 报越界；
2. 跨块前进**不能按 8192**（那只是解压后大小）—— 必须 `块起始 + 2 + 压缩长度`；
3. **不许对打包 ref 做算术**（`ref + 16`）—— 真实镜像里 inode 会**跨元数据块**（实测 `sq_levels` 起点 8180，+16 = 8196 > 8192）→ 必须用能跨块的游标。

**验收（真机 + 逐条对拍，不是抽样推广）**：

| 镜像 | 项数 vs `--describe` | 抽样 30 个文件 vs `--stat` | 耗时（旧 `sqfs2tar`） |
|---|---|---|---|
| `gamedata.sqzy_patch` 2.2GB | 1065 / 1065，零多零缺 | 不一致 **0** | **0.01s**（54.4s） |
| `gamedata.sq_meshes` 1.9GB | 4104 / 4104，零多零缺 | 不一致 **0** | **0.02s** |
| `gamedata.sq_levels` 2.2GB | 3209 / 3209，零多零缺 | 不一致 **0** | **0.02s** |
| `gamedata.sq_textures` 8.3GB（19641 项） | — | — | **0.24s**（99.6s 量级） |

另用**已知全部真实大小**的合成镜像（5001 个文件，lz4 与 gzip 两份）逐条对拍：**零缺失、零多余、零不一致**；
由 `sqfs_pack`（xz）产出的镜像也走同一条路（`run_functional_probe` 的往返锁覆盖）。

**失败必须可见**（消灭"静默 0 B"）：`sqfs_list` 取大小失败时不再 `except: pass`，
而是写模块级 `sqfs_last_error`，`apps/fs_app.py` 在列表日志后打一条 warn。
本机没有 lzo 解码 → lzo 镜像正好是天然的失败样本：**列表仍可用、大小全 0、提示有值**（已落锁）。

**锁**：`run_functional_probe`「SquashFS 真实往返」补一句"成功时 `sqfs_last_error is None`"；
新增「SquashFS 取大小失败**要可见**」。
**注入**（38 → **40** 条）：①读取器返回空 → 往返锁报 `配置/中文名.xml 大小 0 ≠ 13`；
②失败分支不写 `sqfs_last_error` → 可见性锁报"取大小失败却没有任何提示"。
